...
inline float KroneckerDelta(int i, int j) {
...
float P(int i, int a, int b, int l, const std::vector<Eigen::MatrixXf>& r) {
    if (b == l) {
        return GetCenteredElement(r[1], i, 1) * GetCenteredElement(r[l - 1], a, l - 1) -
               GetCenteredElement(r[1], i, -1) * GetCenteredElement(r[l - 1], a, -l + 1);
    } else if (b == -l) {
        return GetCenteredElement(r[1], i, 1) * GetCenteredElement(r[l - 1], a, -l + 1) +
               GetCenteredElement(r[1], i, -1) * GetCenteredElement(r[l - 1], a, l - 1);
    } else {
...
float V(int m, int n, int l, const std::vector<Eigen::MatrixXf>& r) {
    if (m == 0) {
        ...
        return P(1, m - 1, n, l, r) * std::sqrt(1 + d) - P(-1, -m + 1, n, l, r) * (1 - d);
    } else {
        ...
        const float d = KroneckerDelta(m, -1);
        return P(1, m + 1, n, l, r) * (1 - d) + P(-1, -m - 1, n, l, r) * std::sqrt(1 + d);
    }
}
...
void ComputeUVWCoeff(int m, int n, int l, float* u, float* v, float* w) {
    ...
    const float denom = (abs(n) == l ? static_cast<float>(2 * l * (2 * l - 1)) : static_cast<float>((l + n) * (l - n)));
    const float one_over_denom = 1.0f / denom;

    *u = std::sqrt(static_cast<float>((l + m) * (l - m)) * one_over_denom);
    *v =
        0.5f *
        std::sqrt((1.0f + d) * static_cast<float>(l + abs(m) - 1) * (static_cast<float>(l + abs(m))) * one_over_denom) *
        (1.0f - 2.0f * d);
    *w = -0.5f * std::sqrt(static_cast<float>(l - abs(m) - 1) * (static_cast<float>(l - abs(m))) * one_over_denom) *
         (1.0f - d);
}
...
AmbisonicRotator::AmbisonicRotator(int ambisonic_order)
    : ambisonic_order_(ambisonic_order),
      ...
      rotation_matrix_(kNumAmbisonicChannels[ambisonic_order], kNumAmbisonicChannels[ambisonic_order]),
      current_rotation_(WorldRotation::Identity()),
      target_rotation_(2, WorldRotation::Identity()),
      target_rotation_read_index_(0) {
    ...
    for (int l = 1; l <= ambisonic_order_; ++l) {
        const size_t submatrix_size = kNumAmbisonicChannels[l] - kNumAmbisonicChannels[l - 1];
...
void AmbisonicRotator::Process(const float* input_buffer_ptr,
                               size_t num_channels,
                               size_t num_frames,
                               float* output_buffer_ptr) {
    static const WorldRotation kIdentityRotation;
    if (current_rotation_.AngularDifferenceRad(kIdentityRotation) < kRotationQuantizationRad &&
        target_rotation_.at(target_rotation_read_index_.load()).AngularDifferenceRad(kIdentityRotation) <
            kRotationQuantizationRad) {
        for (int c = 0; c < kNumAmbisonicChannels[ambisonic_order_]; ++c) {
            std::copy_n(input_buffer_ptr, num_frames * num_channels, output_buffer_ptr);
        }
        return;
    }
    ...
    const Eigen::Map<const RowMajorMatrixf, Eigen::Aligned> input_matrix(
        input_buffer_ptr, static_cast<int>(num_channels), static_cast<int>(num_frames));
    Eigen::Map<RowMajorMatrixf, Eigen::Aligned> output_matrix(output_buffer_ptr, static_cast<int>(num_channels),
                                                              static_cast<int>(num_frames));

    if (current_rotation_.AngularDifferenceRad(target_rotation_.at(target_rotation_read_index_.load())) <
        kRotationQuantizationRad) {
    ...
    // chunk size.
    for (size_t i = 0; i < num_frames; i += kSlerpFrameInterval) {
        const size_t duration = std::min(num_frames - i, kSlerpFrameInterval);
        const float interpolation_factor = static_cast<float>(i + duration) / static_cast<float>(num_frames);
        UpdateRotationMatrix(
            current_rotation_.slerp(interpolation_factor, target_rotation_.at(target_rotation_read_index_.load())));
        output_matrix.block(0 /* first channel */, i, num_channels, duration) =
            rotation_matrix_ * input_matrix.block(0 /* first channel */, i, num_channels, duration);
    }

    current_rotation_ = target_rotation_.at(target_rotation_read_index_.load());
}

void AmbisonicRotator::SetYawPitchRoll(float yaw, float pitch, float roll) {
    // Because we are processing ambisonic buffer with ACN order, which is arranged as WYZX in AUDIO coordinate and WXYZ
    // in WORLD coordinate. So here we obey the rotation in WORLD coordinate.
    Eigen::AngleAxisf yaw_angle(yaw * M_PI / 180.f, Eigen::Vector3f::UnitY());
    Eigen::AngleAxisf pitch_angle(pitch * M_PI / 180.f, Eigen::Vector3f::UnitX());
    Eigen::AngleAxisf roll_angle(roll * M_PI / 180.f, Eigen::Vector3f::UnitZ());
    auto write_idx = 1 - target_rotation_read_index_.load();
    target_rotation_.at(write_idx) = yaw_angle * pitch_angle * roll_angle;
    target_rotation_read_index_.store(write_idx);
}

void AmbisonicRotator::SetRotationMatrix(const Eigen::Matrix3f& rotation_matrix) {
    auto write_idx = 1 - target_rotation_read_index_.load();
    target_rotation_.at(write_idx) = rotation_matrix;
    target_rotation_read_index_.store(write_idx);
}

void AmbisonicRotator::UpdateRotationMatrix(const Eigen::Quaternion<float>& rotation) {
    rotation_matrices_[1] = rotation.toRotationMatrix();
    ...
    // 0 | 0 0 0 | 0 0 0 0 0 | X X X X X X X
    //
    for (int current_order = 2; current_order <= ambisonic_order_; ++current_order) {
        ComputeBandRotation(current_order, &rotation_matrices_);
        const int index = current_order * current_order;
        const int size =
            static_cast<int>(kNumAmbisonicChannels[current_order] - kNumAmbisonicChannels[current_order - 1]);
        rotation_matrix_.block(index, index, size, size) = rotation_matrices_[current_order];
    }
}
}  // namespace avs3renderer