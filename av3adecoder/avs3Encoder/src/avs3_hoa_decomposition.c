...
static float PYTHAG(float a, float b)
{
    ...
    float at = (float)fabs(a);
    float bt = (float)fabs(b);

    if (at > bt) 
    { 
        ct = bt / at; 
        result = at * (float)sqrt(1.0 + ct * ct);
    }
    else if (bt > 0.f) 
    { 
        ct = at / bt;
        result = bt * (float)sqrt(1.0 + ct * ct); 
    }
    else 
    {
        result = 0.f;
...
void Avs3HoaSVD(float a[HOA_LEN_FRAME48k][L_HOA_BASIS_ROWS], int m, int n, float* w, float v[HOA_LEN_FRAME48k][L_HOA_BASIS_ROWS])
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        anorm = MAX(anorm, (float)(fabs((float)w[i]) + fabs(rv1[i])));
    }
    ...
    {                             /* loop over singular values */
        ...
        {                         /* loop over allowed iterations */
            ...
            {                     /* test for splitting */
                ...
                if ((float)fabs((float)w[nm]) + anorm == anorm)
                    break;
            }
            if (flag)
            {
