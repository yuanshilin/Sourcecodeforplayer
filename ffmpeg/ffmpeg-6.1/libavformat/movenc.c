...
static int mov_write_hdrvivid_cuvv_tag(AVFormatContext *s, AVIOContext *pb, AVHDRVIVIDDecoderCUVVRecord *vivid)
{
    //uint16_t buf[ISOM_HDRVIVID_CUVV_SIZE/2];
    uint8_t buf[ISOM_HDRVIVID_CUVV_SIZE];

    avio_wb32(pb, 30); /* size = 8 + 22 */
    ffio_wfourcc(pb, "cuvv");

    ff_isom_put_cuvv(s, buf, vivid);
    avio_write(pb, buf, sizeof(buf));
...
static int mov_write_video_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    if(track->st->cuvv)
      strcpy(compressor_name,"HDR Vivid Video     ");
    ...
    if (track->mode == MODE_MP4 && mov->fc->strict_std_compliance <= FF_COMPLIANCE_UNOFFICIAL) {
        AVHDRVIVIDDecoderCUVVRecord *vivid=(AVHDRVIVIDDecoderCUVVRecord *)av_stream_get_side_data(track->st, AV_PKT_DATA_VIVID_CUVV, NULL);
//	printf("In movenc.c func %s line %d track->st->cuvv=%d\n",__func__,__LINE__,track->st->cuvv);
#if 0 /*should get from command line parameters that it is now HDR Vivid encoder!*/
        if(vivid)
             mov_write_hdrvivid_cuvv_tag(s, pb, vivid);
		printf("In movenc.c func %s line %d av_stream_get_side_data() return vivid=%p\n",__func__,__LINE__,vivid);
#else /*here just a shortcut to feed the cuvv box info.*/
        if(track->st->cuvv==1){
             vivid=malloc(sizeof(AVHDRVIVIDDecoderCUVVRecord));
             vivid->cuva_version_map=0x1;
             vivid->terminal_provide_code=0x0004;
             vivid->terminal_provide_oriented_code=0x0005;
             mov_write_hdrvivid_cuvv_tag(s, pb, vivid);
        }
#endif
    }
