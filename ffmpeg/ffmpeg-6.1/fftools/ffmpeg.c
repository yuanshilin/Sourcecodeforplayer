...
extern FILE *fp_vmetafile;
extern int vivid_pass;
extern int vivid_dump;
...
static int process_input(int file_index)
{
    ...
    if(vmetafile!=NULL){
      buf = malloc(MAX_BINARY_METADATA_LEN);
      do{
        line = vmeta_read_one_line(fp_vmetafile);
      }while(line[0]=='#');
      get_vivid_metadata_from_one_line_string(line,buf,&leng);
      av_packet_new_side_data(pkt,AV_PKT_DATA_DYNAMIC_HDR_VIVID,HDRVIVID_SIDE_DATA_LEN/*leng*/);
      memcpy(pkt->side_data->data, buf,HDRVIVID_SIDE_DATA_LEN/*leng*/);
    }
...
int main(int argc, char **argv)
{
    ...
    if(argc>=2&&(!strcmp(argv[1],"check")||!strcmp(argv[1],"show"))){
      ret=check_cmd(argc,argv);
      if(ret==0){
        int linetocheck=argc==4?atoi(argv[3]):0;
        bool show_metadata_details=strcmp(argv[1],"check")?1:0; //0 --> check; 1 --> show
        ret=check_vivid_metadata_file(argv[2],linetocheck,show_metadata_details);
        if(ret!=0)
          printf("Bad metadata in file %s !\n",argv[2]);
      }
      exit(1);
    }
    init_dynload();
    ...
    if(vmetafile!=NULL){
      ret=check_vivid_metadata_file(vmetafile,0,0);
      if(ret!=0){
        printf("Bad metadata file, please modify it and then try again!\n");
	    exit(1);
      }

      fp_vmetafile = fopen(vmetafile, "r");
      if (fp_vmetafile == NULL){
        printf("In ffmpeg.c func %s line %d  metadata file open KO!\n",__func__,__LINE__ );
        exit(EXIT_FAILURE);
      }else{
        printf("In ffmpeg.c func %s line %d  metadata file open OK!\n",__func__,__LINE__ );
      }
    }
    current_time = ti = get_benchmark_time_stamps();
    printf("In fftools/ffmpeg.c func %s line %d,nb_output_files=%d nb_input_files=%d\n",__func__,__LINE__,nb_output_files,nb_input_files);
    ret = transcode(&err_rate_exceeded);
    if(vmetafile!=NULL){
      if(fclose(fp_vmetafile))
        printf("In ffmpeg.c func %s line %d  metadata file close KO!\n",__func__,__LINE__ );
    }
