...
static void print_dynamic_hdr_vivid_to_file(WriterContext *w, const AVDynamicHDRVivid *metadata)
{
   static int number_of_metadata=0;
   int ssc;//metadata->system_start_code
   if (!metadata){
        return;
   }
   number_of_metadata++;
   fprintf(fp_metadatafile,"%d ", number_of_metadata);
   ssc = metadata->system_start_code;
   if(ssc==0x01||ssc==0x02||ssc==0x03||ssc==0x04||ssc==0x05||ssc==0x06||ssc==0x07){
      fprintf(fp_metadatafile,"%d ", metadata->system_start_code);

      for (int n = 0; n < metadata->num_windows; n++) {
      const AVHDRVividColorTransformParams *params = &metadata->params[n];
         fprintf(fp_metadatafile,"%d %d %d %d ",params->minimum_maxrgb.num,params->average_maxrgb.num,params->variance_maxrgb.num,params->maximum_maxrgb.num);
      }
      for (int n = 0; n < metadata->num_windows; n++) {
      const AVHDRVividColorTransformParams *params = &metadata->params[n];
        fprintf(fp_metadatafile,"%d ",params->tone_mapping_mode_flag);
        if (params->tone_mapping_mode_flag){
            fprintf(fp_metadatafile,"%d ",params->tone_mapping_param_num - 1);
            for (int i = 0; i < params->tone_mapping_param_num ; i++) {
                const AVHDRVividColorToneMappingParams *tm_params = &params->tm_params[i];

                fprintf(fp_metadatafile,"%d ", tm_params->targeted_system_display_maximum_luminance.num);
                fprintf(fp_metadatafile,"%d ", tm_params->base_enable_flag);
                if (tm_params->base_enable_flag) {
                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_m_p.num);
                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_m_m.num);
                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_m_a.num);
                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_m_b.num);
                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_m_n.num);

                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_k1);
                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_k2);
                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_k3);
                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_Delta_enable_mode);
                    fprintf(fp_metadatafile,"%d ",tm_params->base_param_Delta.num);
                }
                fprintf(fp_metadatafile,"%d ",tm_params->three_Spline_enable_flag);
                if (tm_params->three_Spline_enable_flag) {
                    fprintf(fp_metadatafile,"%d ",tm_params->three_Spline_num - 1);

                    for (int j = 0; j < tm_params->three_Spline_num; j++) {
                        const AVHDRVivid3SplineParams *three_spline = &tm_params->three_spline[j];
                        fprintf(fp_metadatafile,"%d ",three_spline->th_mode);
                        if (three_spline->th_mode == 0 || three_spline->th_mode == 2)
                            fprintf(fp_metadatafile,"%d ",three_spline->th_enable_mb.num);
                        fprintf(fp_metadatafile,"%d ",three_spline->th_enable.num);
                        fprintf(fp_metadatafile,"%d ",three_spline->th_delta1.num);
                        fprintf(fp_metadatafile,"%d ",three_spline->th_delta2.num);
                        fprintf(fp_metadatafile,"%d ",three_spline->enable_strength.num);
                    }
                }
            }
         } //if tone_mapping_mode_flag

        fprintf(fp_metadatafile,"%d ", params->color_saturation_mapping_flag);

        if (params->color_saturation_mapping_flag) {
            fprintf(fp_metadatafile,"%d ", params->color_saturation_num);
            for (int i = 0; i < params->color_saturation_num; i++) {
                fprintf(fp_metadatafile,"%d ", params->color_saturation_gain[i].num);
            }
         } //end for color_saturation_mapping_flag
      } //2nd for num_windows ends
   } //if ssc valid ends
   fprintf(fp_metadatafile, "\n");
}
                                  ...
                                  const AVStream *stream)
{
    ...
    for (int i = 0; i < frame->nb_side_data; i++) {
        ...
        if (sd->type == AV_FRAME_DATA_DISPLAYMATRIX && sd->size >= 9*4) {
            ...
            print_dynamic_hdr_vivid_to_file(w, metadata);
...
int main(int argc, char **argv)
{
    ...
    init_dynload();
    if(argc==3){
	  metadatafile=malloc(strlen(argv[2])+strlen(".metadata.ffprobe.txt"));
      strcpy(metadatafile,argv[2]);
      strcat(metadatafile,".metadata.ffprobe.txt");
      fp_metadatafile = fopen(metadatafile, "wb+");
      if(fp_metadatafile==NULL){
        printf("In func %s line %d, fopen(metadatafile) KO\n",__func__,__LINE__);
        return 1;
      }
	}
    ...
    if(argc==3){
      if(fclose(fp_metadatafile))
        printf("\nIn func %s line %d,fclose(fp_metadatafile) error!\n",__func__,__LINE__);
    }
