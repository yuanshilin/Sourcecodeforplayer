...
static int set_side_data(HEVCContext *s)
{
    ...
    if(s->sei.common.dynamic_hdr_vivid.info  && s->sei.common.dynamic_hdr_vivid.info->size)
    {
        AVDynamicHDRVivid *vivid_meta_ctx=(AVDynamicHDRVivid *)s->sei.common.dynamic_hdr_vivid.info->data;
        AVDynamicHDRVivid *vivid_meta_frame   = av_dynamic_hdr_vivid_create_side_data(out);
        if(s->sei.common.dynamic_hdr_vivid.info->size == sizeof(AVDynamicHDRVivid))
        {
             memcpy(vivid_meta_frame,vivid_meta_ctx,sizeof(AVDynamicHDRVivid));
             vivid_meta_frame->system_start_code = vivid_meta_ctx->system_start_code;
            //  printf("add sidedata frame pts = %d,  vivid:[%d,%d,%d,%d]\n",
            //  out->pts, vivid_meta_frame->params[0].minimum_maxrgb.num,
            //            vivid_meta_frame->params[0].average_maxrgb.num, 
            //            vivid_meta_frame->params[0].variance_maxrgb.num,   
            //            vivid_meta_frame->params[0].maximum_maxrgb.num);
        }
        if(out->nb_side_data>0)
        {
            out->side_data[out->nb_side_data-1]->type = AV_FRAME_DATA_DYNAMIC_HDR_VIVID;
        }
