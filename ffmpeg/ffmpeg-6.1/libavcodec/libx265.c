...
static int vivid_meda_dump(int framenum,  AVDynamicHDRVivid *pMeta, char *metafile)
{
    ...
    sprintf(stemp, "%d %d %d %d %d %d %d", frame_num_int,
            pMeta->system_start_code,pVidiParam->minimum_maxrgb.num,pVidiParam->average_maxrgb.num,pVidiParam->variance_maxrgb.num,pVidiParam->maximum_maxrgb.num,pVidiParam->tone_mapping_mode_flag);
...
static av_cold int libx265_encode_init(AVCodecContext *avctx)
{
    ...
    ctx->vivid_embed             = avctx->vivid_embed;
    ctx->vivid_dump              = avctx->vivid_dump;
...
static int gennerate_vivid_metadata_from_struct(AVDynamicHDRVivid* vivid_frame, uint8_t *data, int *size)
{
  ...
  put_bits(pb, 8,vivid_frame->system_start_code);
  count += 8;

  // T/UWA 005.1-2022, table 11
  if (vivid_frame->system_start_code >= 0x01 && vivid_frame->system_start_code <= 0x07) {
    vivid_frame->num_windows = 1;
    for (int w = 0; w < vivid_frame->num_windows; w++) {
      AVHDRVividColorTransformParams *params = &vivid_frame->params[w];
    ...
    for (int w = 0; w < vivid_frame->num_windows; w++) {
      ...
      if (params->tone_mapping_mode_flag) {
        put_bits(pb, 1,params->tone_mapping_param_num-1)/* + 1*/;
        count += 1;
        for (int i = 0; i < params->tone_mapping_param_num ; i++) {
          ...
          if (tm_params->three_Spline_enable_flag) {
            ...
            int three_sp_num = tm_params->three_Spline_num-1;
            put_bits(pb, 1,three_sp_num)/* + 1*/;
            count += 1;
            for (int j = 0; j < three_sp_num+1; j++) {
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        AVFrameSideData *side_data = av_frame_get_side_data(pic, AV_FRAME_DATA_DYNAMIC_HDR_VIVID);
        if(side_data)
        {
            if(side_data->size == sizeof(AVDynamicHDRVivid))
            {
                if(ctx->vivid_embed)
                {
                    AVDynamicHDRVivid *vivid_meta_frame = (AVDynamicHDRVivid*)side_data->data;
                    AVDynamicHDRVivid vivid_meta;
                    memcpy(&vivid_meta,vivid_meta_frame,sizeof(AVDynamicHDRVivid));
                    char* sei_data =  (char*)av_mallocz(256);
                    int sei_size = 0;
                    if(ctx->vivid_dump)
                    {
                    static int enc_count = 0;
                    enc_count++;
                    vivid_meda_dump(enc_count,&vivid_meta,"vivid_encode_dump.txt");
                    }
                    gennerate_vivid_metadata_from_struct(&vivid_meta,sei_data,&sei_size);
                    // sei_size = 0;
                    if(sei_size>0)
                    {
                        void *tmp;
                        ...
                        if (!tmp) {
                            ...
                            sei_payload->payloadType = SEI_TYPE_USER_DATA_REGISTERED_ITU_T_T35;
                            sei->numPayloads++;
                            // printf("x265insert vivid pts = %d,  vivid:[%d,%d,%d,%d],metasize=%d,numpayload=%d\n",
                            //         pic->pts, vivid_meta_frame->params[0].minimum_maxrgb.num,
                            //         vivid_meta_frame->params[0].average_maxrgb.num, 
                            //         vivid_meta_frame->params[0].variance_maxrgb.num,   
                            //         vivid_meta_frame->params[0].maximum_maxrgb.num,sei_size,sei->numPayloads);
                    }else
                    {
                        av_free(sei_data);
                    }
            ...
            }else  //read from file 
            {
                if (ctx->vmeta) {
                    ...
                    ret = ff_alloc_vmeta_sei(pic, 0, &sei_data, &sei_size);
                    if (ret < 0) {
                        av_log(ctx, AV_LOG_ERROR, "Not enough memory for HDR Vivid, skipping\n");
                    } else if (sei_data) {
                        void *tmp;
                        x265_sei_payload *sei_payload;
...
static const AVOption options[] = {
    ...
    { "vmeta",	     "Use HDR vivid metadata (if available)",                                       OFFSET(vmeta),    AV_OPT_TYPE_BOOL,   { .i64 = 1 }, 0, 1, VE }, //set default value to 1, no worry, as if not for vivid, the sei_data is empty, so still no metadata encoding
