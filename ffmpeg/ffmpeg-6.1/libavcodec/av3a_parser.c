                           ...
                           int buf_size, AVCodecContext *avctx)
{
    ...
    if (buf[0] == 0x0 && buf[1] == 0x0 && buf[2] == 0x1) {
        if (buf[3] == AVS3_SEQ_START_CODE) {
            ...
            //int chroma = get_bits(&gb, 2);
            //int sampe_precision = get_bits(&gb, 3);
            skip_bits(&gb, 5);
            //skip_bits(&gb, 47);

            avctx->width = width;
            avctx->height = height;
            //avctx->chroma_sample_location = chroma;
...
static int av3a_parse(AVCodecParserContext *s, AVCodecContext *avctx,
